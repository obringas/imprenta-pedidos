...
**Sin pagar:** pedidos con saldo pendiente (`saldo > 0`)  
...
### Informes
- La pestana `Sin pagar` lista todos los pedidos con `saldo > 0`, sin importar el estado de impresion.
- La pestana `Impresos sin pagar` lista pedidos con `estado_impresion = 'Impreso'` y `saldo > 0`.
- Desde informes, la accion `Marcar pagado` debe forzar pago total: `estado_pago = 'Pagado'`, `monto_cobrado = precio_cobrado`, `saldo = 0`.
...
- La tabla `Avance por libro` debe ocultar libros sin trabajo abierto: no mostrar libros 100% cerrados ni libros sin pedidos.
- En informes operativos, el filtro `Libro` debe mostrar solo libros con resultados pendientes para la pestana actual.
...
- [x] `Hecho` Ajustar `Sin pagar` para mostrar todos los pedidos con saldo pendiente, aunque todavia falten imprimir.
- [x] `Hecho` Agregar `Impresos sin pagar` para ver solo pedidos impresos con saldo pendiente.
- [x] `Hecho` Hacer que `Marcar pagado` en informes deje siempre el saldo en cero.
- [x] `Hecho` Ocultar libros 100% cerrados o sin pedidos del avance por libro.
- [x] `Hecho` Limitar el filtro de libro en informes a libros con resultados pendientes en la vista actual.
